...
def worker(Re):
    ...
    integ = "RK4-3-8-point-{}".format(current_p._identity[0])

    def run_fname():
        nsteps = int(tend / probDescription.get_dt())
        # here add your scheme:
        # ------------------------
        is_stable = RK4_taylor_vortex(steps=nsteps, return_stability=True, name='3/8',
                                                        guess=None, project=[1,1,1], alpha=0.999)
...
if __name__ == "__main__":
    ...
    with open("RK4-3-8-stability.txt", 'w') as output_file:
        for id in range(1, n + 1):
            with open("RK4-3-8-point-{}-stability.txt".format(id), 'r') as input_file:
