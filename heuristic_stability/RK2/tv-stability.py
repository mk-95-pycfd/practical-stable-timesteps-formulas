...
def worker(Re):
    ...
    integ = "RK2-heun-stability-point-{}".format(current_p._identity[0])

    def run_fname():
        nsteps = int(tend / probDescription.get_dt())
        # here add your scheme:
        # ------------------------
        is_stable = RK2_taylor_vortex(steps=nsteps, return_stability=True, name='heun',
                                                        guess=None, project=[1], alpha=0.999)
...
if __name__ == "__main__":
    ...
    with open("RK2-heun-stability.txt", 'w') as output_file:
        for id in range(1, n + 1):
            with open("RK2-heun-stability-point-{}-stability.txt".format(id), 'r') as input_file:
